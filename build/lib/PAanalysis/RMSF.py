...
def local_RMSF(grofile, trajfile, frame_iterator, residuenames=None):
    """
    atomistic simulation
    Root mean square fluctuation of the peptide backbone
    displacement wrt mean configuration
    ...
    print(type(residuenames) != type(None))
    ...
    query_args = dict(mode='nearest', num_neighbors=20, r_max=radius, exclude_ii=True)
